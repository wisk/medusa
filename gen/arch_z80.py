        ...
        class OprdVst(ast.NodeVisitor):
            ...
            def visit_Call(self, node):
                ...
                if func_name == 'read':
                    ...
                    if len(func_args) == 1:
                        self.var_expr.append('Expr::MakeInt(%d, Imm)' % read_size)

                    else:
                        self.var_expr.append('Expr::MakeInt(%d, SignExtend<s64, %d>(Imm))' % (func_args[1], read_size))
                ...
                if func_name == 'cst':
                    ...
                    self.var_expr.append('Expr::MakeInt(%d, %#x)' % (cst_size, cst_value))
                    return self.var_expr[-1]
