...
std::string X86Architecture::X86CpuContext::ToString(void) const
{
  ...
  {
  case X86_Bit_16:
    return (boost::format("ax: %04x bx: %04x cx: %04x dx: %04x\nsi: %04x di: %04x sp: %04x bp: %04x ip: %04x")
      % m_Context.a.w % m_Context.b.w % m_Context.c.w % m_Context.d.w % m_Context.si.w % m_Context.di.w % m_Context.sp.w % m_Context.bp.w % m_Context.ip.w).str();

  case X86_Bit_32:
    return (boost::format("eax: %08x ebx: %08x ecx: %08x edx: %08x\nesi: %08x edi: %08x esp: %08x ebp: %08x eip: %08x")
      % m_Context.a.e % m_Context.b.e % m_Context.c.e % m_Context.d.e % m_Context.si.e % m_Context.di.e % m_Context.sp.e % m_Context.bp.e % m_Context.ip.e).str();

  case X86_Bit_64:
    return (boost::format("rax: %016x rbx: %016x rcx: %016x rdx: %016x\nrsi: %016x rdi: %016x rsp: %016x rbp: %016x rip: %016x")
      % m_Context.a.r % m_Context.b.r % m_Context.c.r % m_Context.d.r % m_Context.si.r % m_Context.di.r % m_Context.sp.r % m_Context.bp.r % m_Context.ip.r).str();
