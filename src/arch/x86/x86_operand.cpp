  ...
  case 0x6:
    {
      ...
      if ((ModRm.Mod()) == 0x0)
      {
        ...
        spOff = Expr::MakeConstInt(16, Disp16);
  ...
  case 0x1:
    {
      ...
      return Expr::MakeBinOp(OperationExpression::OpAdd, spOff, Expr::MakeConstInt(16, static_cast<u16>(SignExtend<s16, 8>(Disp8))));
      break;
    }
  case 0x2:
    {
      ...
      rBinStrm.Read(Offset + 1, Disp16);
      OprdLen += sizeof(Disp16);
      return Expr::MakeBinOp(OperationExpression::OpAdd, spOff, Expr::MakeConstInt(16, Disp16));
      break;
    }
  default:
    return nullptr;
...
static Expression::SPType __DecodeSib32(CpuInformation* pCpuInfo, BinaryStream const& rBinStrm, TOffset Offset, Instruction const& rInsn, u8& rOprdLen)
{
    ...
    case 0x0:
      {
        ...
        spDisp = Expr::MakeConstInt(32, Disp32);
    ...
    case 0x1:
      {
        ...
        spDisp = Expr::MakeConstInt(32, static_cast<u32>(SignExtend<s32, 8>(Disp8)));
  ...
  auto spScale = Expr::MakeConstInt(32, aScale[Sib.Scale()]);
...
static Expression::SPType __DecodeModRmAddress32(CpuInformation* pCpuInfo, BinaryStream const& rBinStrm, TOffset Offset, Instruction const& rInsn, u8& rOprdLen)
{
  ...
  else if (ModRm.Mod() == 0x0 && ModRm.Rm() == 0x5)
  {
    ...
    rBinStrm.Read(Offset + sizeof(ModRm), Disp32);
    rOprdLen += sizeof(Disp32);
    return Expr::MakeConstInt(32, Disp32);
  }
...
static Expression::SPType __DecodeSib64(CpuInformation* pCpuInfo, BinaryStream const& rBinStrm, TOffset Offset, Instruction& rInsn, u8& rOprdLen)
{
    ...
    case 0x0:
      {
        u32 Disp32;
        rBinStrm.Read(Offset + sizeof(ModRm), Disp32);
        spDisp = Expr::MakeConstInt(64, static_cast<u64>(SignExtend<s64, 32>(Disp32)));
        rOprdLen += sizeof(Disp32);
        break;
      }
    ...
    case 0x1:
      {
        ...
        u8 Disp8;
        rBinStrm.Read(Offset + sizeof(ModRm), Disp8);
        spDisp = Expr::MakeConstInt(64, static_cast<u64>(SignExtend<s64, 8>(Disp8)));
        rOprdLen += sizeof(Disp8);
        break;
      }
  ...
  auto spSecReg = pRegIndex[Sib.Index()] ? Expr::MakeId(pRegIndex[Sib.Index()], pCpuInfo) : nullptr;
  auto spScale = Expr::MakeConstInt(64, aScale[Sib.Scale()]);
...
static Expression::SPType __DecodeModRmAddress64(CpuInformation* pCpuInfo, BinaryStream const& rBinStrm, TOffset Offset, Instruction& rInsn, u8& rOprdLen)
{
  ...
  else if (ModRm.Mod() == 0x0 && ModRm.Rm() == 0x5)
  {
    ...
    rBinStrm.Read(Offset + sizeof(ModRm), Disp32);
    rOprdLen += sizeof(Disp32);
    return Expr::MakeConstInt(64, Disp32);
  }
    ...
    // Handle [eip+Disp32] or [rip+Disp32] (x86_64 only)
    {
      ...
      if (ModRm.Mod() == 0x00 && ModRm.Rm() == 0x05)
      {
        ...
        if (Is64)
          ...
          /**/Expr::MakeConstInt(64, SignExtend<s64, 32>(Disp32)));
        else
          spExpr = Expr::MakeBinOp(OperationExpression::OpAdd,
          /**/Expr::MakeId(Reg, pCpuInfo),
          /**/Expr::MakeConstInt(32, Disp32));
        break;
      }
    }
...
Expression::SPType X86Architecture::__Decode_Ap(BinaryStream const& rBinStrm, TOffset Offset, Instruction& rInsn, u8 Mode)
{
  auto Ap16 = [&](void) -> Expression::SPType
  {
    ...
    auto spConstSeg = Expr::MakeConstInt(16, Seg);
    auto spConstOff = Expr::MakeConstInt(16, Off);
...
Expression::SPType X86Architecture::__Decode_Ibs(BinaryStream const& rBinStrm, TOffset Offset, Instruction& rInsn, u8 Mode)
{
  ...
  return Expr::MakeConstInt(BitSize, SignExtend<s64, 8>(Value));
}
...
Expression::SPType X86Architecture::__Decode_Jb(BinaryStream const& rBinStrm, TOffset Offset, Instruction& rInsn, u8 Mode)
{
  ...
  return Expr::MakeBinOp(OperationExpression::OpAdd,
    ...
    Expr::MakeConstInt(RegPcSize, SignExtend<s64, 8>(Value)));
}
  ...
  case 16:
    {
      ...
      return Expr::MakeBinOp(OperationExpression::OpAdd,
        Expr::MakeId(RegPc, &m_CpuInfo),
        Expr::MakeConstInt(RegPcSize, SignExtend<s64, 16>(Value)));
    }
  ...
  case 32: case 64:
    {
      ...
      return Expr::MakeBinOp(OperationExpression::OpAdd,
        Expr::MakeId(RegPc, &m_CpuInfo),
        Expr::MakeConstInt(RegPcSize, SignExtend<s64, 32>(Value)));
    }
  ...
  case 16:
    {
      ...
      if (!rBinStrm.Read(Offset, Value))
        return nullptr;
      spOff = Expr::MakeConstInt(16, Value);
      rInsn.Length() += sizeof(Value);
      break;
    }
  ...
  case 32:
    {
      ...
      if (!rBinStrm.Read(Offset, Value))
        return nullptr;
      spOff = Expr::MakeConstInt(32, Value);
      rInsn.Length() += sizeof(Value);
      break;
    }
  ...
  case 64:
    {
      ...
      if (!rBinStrm.Read(Offset, Value))
        return nullptr;
      spOff = Expr::MakeConstInt(64, Value);
      rInsn.Length() += sizeof(Value);
      break;
    }
...
Expression::SPType X86Architecture::__Decode_Uoq(BinaryStream const& rBinStrm, TOffset Offset, Instruction& rInsn, u8 Mode)
{
  Expression::SPType spOprd;
  u8 OprdLen = 0;
  auto const ModRm = __GetModRm(rBinStrm, Offset);
