...
struct ConfigGetter : public boost::static_visitor<>
{
	...
	{
		QCheckBox* toto = qobject_cast<QCheckBox *>(_widgets[rBool.GetName()].second);
		m_rCfg.Set(rBool.GetName(), toto->isChecked());
	}

	void operator()(medusa::ConfigurationModel::NamedEnum const& rEnum) const
	{
		QComboBox* toto = qobject_cast<QComboBox *>(_widgets[rEnum.GetName()].second);
		m_rCfg.Set(rEnum.GetName(), toto->itemData(toto->currentIndex()).toUInt());
	}
};
...
public:
	bool				getSelection(medusa::Loader::Ptr & loader, medusa::Architecture::Ptr & architecture);
