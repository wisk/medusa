...
{
  std::cerr << "-------------------- DOCUMENT: ADDLABEL" << std::endl;
  if (m_spDatabase == nullptr)
    return;
  ...
  std::cerr << "-------------------- DOCUMENT: ADDLABEL: STEP1" << std::endl;
  ...
  {
    std::cerr << "-------------------- DOCUMENT: ADDLABEL: STEP2" << std::endl;
    if (OldLbl.IsAutoGenerated())
      Force = true;

    if (!Force)
      return;
    std::cerr << "-------------------- DOCUMENT: ADDLABEL: STEP3" << std::endl;

    if (OldLbl == rLabel)
      return;

    std::cerr << "-------------------- DOCUMENT: ADDLABEL: STEP4" << std::endl;
    if (!m_spDatabase->RemoveLabel(rAddr))
      return;

    m_LabelUpdatedSignal(rAddr, OldLbl, true);
  }
  std::cerr << "-------------------- DOCUMENT: ADDLABEL: STEP5" << std::endl;
...
{
  std::cerr << "---------------------- Document : RemoveLabel STEP 1" << std::endl;
  if (m_spDatabase == nullptr)
    return;
  std::cerr << "---------------------- Document : RemoveLabel STEP 2" << std::endl;
  Label CurLbl;
  m_spDatabase->GetLabel(rAddr, CurLbl);
...
bool Document::SetCell(Address const& rAddr, Cell::SPType spCell, bool Force)
{
  ...
  std::cerr << "---------------------- First SetCell: SetCellData" << std::endl;
  if (!m_spDatabase->SetCellData(rAddr, *spCell->GetData(), ErasedAddresses, Force))
    return false;
  std::cerr << "---------------------- First SetCell: SetCellData -- BOUM" << std::endl;
...
bool Document::SetCellWithLabel(Address const& rAddr, Cell::SPType spCell, Label const& rLabel, bool Force)
{
  if (m_spDatabase == nullptr)
    return false;
  Address::List ErasedAddresses;
  std::cerr << "---------------------- Second SetCellWithLabel : SetCellData" << std::endl;
  if (!m_spDatabase->SetCellData(rAddr, *spCell->GetData(), ErasedAddresses, Force))
    return false;
...
{
  std::cerr << "---------------------- REMOVE LABEL IF NEEDED : STEP1" << std::endl;
  auto Lbl = GetLabelFromAddress(rAddr);
  if (Lbl.GetType() == Label::Unknown)
    return;
  std::cerr << "---------------------- REMOVE LABEL IF NEEDED : STEP2" << std::endl;
  if (Lbl.GetType() & (Label::Exported | Label::Imported))
    return;
  std::cerr << "---------------------- REMOVE LABEL IF NEEDED : STEP3" << std::endl;
